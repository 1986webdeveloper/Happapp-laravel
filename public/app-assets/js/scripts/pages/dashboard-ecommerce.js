...
var month_wise_video_viewers = $.parseJSON(month_wise_video_viewer, true);
var countView = [];
var monthName = [];
var defaultVal = [0];
    for (var key in month_wise_video_viewers) {
        var count_viewer = month_wise_video_viewers[key].count_total;
        var month_name = month_wise_video_viewers[key].month_name;
            countView.push(count_viewer);
            monthName.push(month_name);            
    }
    var countView = countView.concat(defaultVal);


var day_wise_video_viewers = $.parseJSON(day_wise_video_viewer, true);
var dayCountView = [];
var dayName = [];
var dayDefaultVal = [0];
    for (var key in day_wise_video_viewers) {
        var day_count_viewer = day_wise_video_viewers[key].count_total;
        var day_name = day_wise_video_viewers[key].day_name;
            dayCountView.push(day_count_viewer);
            dayName.push(day_name);            
    }
    var dayCountView = dayCountView.concat(dayDefaultVal); 


var week_wise_video_viewer = $.parseJSON(week_wise_video_viewer, true);
var weekCountView = [];
var weekName = [];
var weekDefaultVal = [0];
    for (var key in week_wise_video_viewer) {
        var week_count_viewer = week_wise_video_viewer[key].count_total;
        var week_name = week_wise_video_viewer[key].weeks;
            weekCountView.push(week_count_viewer);
            weekName.push(week_name);            
    }
    var weekCountView = weekCountView.concat(weekDefaultVal);    
...
(function(window, document, $) {
    ...
    *************************************************/
    (function () {
      ...
      var DayLabelList = dayName;
      var DaySeries1List = {
        name: "series-1",
        // data: [0, 10, 2600, 6100, 2600, 6500, 3200, 6800]
        data: dayCountView
